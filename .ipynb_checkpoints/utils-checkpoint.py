...
##########################Step 6
def step7_lr_div_dir_delta(df_asy_train, ws = 1, y_col = 'aar_1%'):
    d2 = generate_model_data(df_asy_train, window_size=ws, y_col = y_col, delta_precentage = True,drop_08_09 = True, print_ = 0)
    d2 = pd.get_dummies(data = d2,    columns = ['div_direction'],\
                                      drop_first = False)
    lr_features = ['delta_%_t-4', 'div_direction_1', 'div_direction_-1', y_col]
    d2 = d2[d2['div_direction_0']==0][lr_features]
    m = regression_models.run_linear_reg(d2,
                                         y_col = y_col, print_r2=False)
    display(m.summary2())
##########################Step 7
